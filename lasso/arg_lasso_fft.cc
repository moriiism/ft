#include "arg_lasso_fft.h"

// public

void ArgValLassoFft::Init(int argc, char* argv[])
{
    progname_ = "lasso_fft";
...
void ArgValLassoFft::Print(FILE* fp) const
{
...
void ArgValLassoFft::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValLassoFft::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "infile  gtifile  time_lo  time_up  lambda  "
            "outdir  outfile_head \n",
            progname_.c_str());
    exit(1);
}
